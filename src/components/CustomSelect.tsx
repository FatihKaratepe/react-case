...
const CustomSelect = ({ options, disabled = false, filter = false, sorting = false, multiple = false, prefixIcon, selectedIcon, bindLabel, placeHolder, onChange }: CustomSelectProps) => {

  const [showMenu, setShowMenu] = useState(false); // Controls the visibility of the dropdown menu
  const [selectedValue, setSelectedValue] = useState(multiple ? [] as any[] : null as any); // Stores the selected value(s)
  ...
  const inputRef = useRef<HTMLDivElement>(); // Reference to the custom select input element
  ...
  const isSelected = (option: any) => {
    if (multiple) return selectedValue!.filter((o: any) => o.value === option.value).length > 0;
  ...
  const onItemClick = (option: any) => {
    let newValue;
    if (multiple) {
      if (selectedValue.findIndex((o: any) => o.value === option.value) >= 0) newValue = removeOption(option);
  ...
  return (
    <>
      <div className={`customSelectContainer`} onClick={() => { handleInputClick() }}>
        ...
        <div className="customSelectValueContainer">
          <input ref={searchRef} onChange={(e) => onSearch(e)} type="text" placeholder={!selectedValue || selectedValue.length === 0 ? placeHolder : ''} className="customSelectSearchInput" disabled={filter !== true} />
